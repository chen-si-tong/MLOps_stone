...
@hydra.main(config_path="/Users/chensitong/MLOps/project/MLOps_test/testpar", config_name="config.yaml")
def train(config):
    ...
    for epoch in range(hparams["epochs"]):
        ...
        for batch_idx, (x, _) in enumerate(train_loader):
            ...
            loss.backward()
            optimizer.step()
        log.info(print(f"Epoch {epoch+1} complete!,  Average Loss: {overall_loss / (batch_idx*hparams['batch_size'])}"))
    log.info("Finish!!")
